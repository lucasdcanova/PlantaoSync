...
import { DEMO_FINANCIAL_CYCLES } from '@/lib/demo-data'
import { formatCurrency } from '@/lib/utils'

export default function FinancesPage() {
  const projected = DEMO_FINANCIAL_CYCLES.reduce((sum, cycle) => sum + cycle.projectedCost, 0)
  const confirmed = DEMO_FINANCIAL_CYCLES.reduce((sum, cycle) => sum + cycle.confirmedCost, 0)
  const paid = DEMO_FINANCIAL_CYCLES.reduce((sum, cycle) => sum + cycle.paidCost, 0)
  const variance = DEMO_FINANCIAL_CYCLES.reduce((sum, cycle) => sum + cycle.variance, 0)

  const statusClassName: Record<(typeof DEMO_FINANCIAL_CYCLES)[number]['status'], string> = {
    'Em fechamento':
    ...
    <>
      ...
      <div className="space-y-6 p-6">
        ...
        <section className="rounded-2xl border border-border bg-card p-6 shadow-card">
          <h2 className="font-display text-lg font-bold text-foreground">Ciclos financeiros</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Comparativo de previsão, confirmado, pago e desvio por ciclo de cobertura.
          </p>

          <div className="mt-5 space-y-3">
            {DEMO_FINANCIAL_CYCLES.map((cycle) => (
              <article key={cycle.id} className="rounded-xl border border-border bg-background p-4 md:p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="font-medium text-foreground">{cycle.cycle}</h3>
                    <p className="text-xs text-muted-foreground">Controle de fechamento assistencial e contábil</p>
                  </div>
                  <Badge className={statusClassName[cycle.status]}>{cycle.status}</Badge>
                </div>

                <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Projetado</p>
                    <p className="mt-1 text-foreground">{formatCurrency(cycle.projectedCost)}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Confirmado</p>
                    <p className="mt-1 text-foreground">{formatCurrency(cycle.confirmedCost)}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Pago</p>
                    <p className="mt-1 text-foreground">{formatCurrency(cycle.paidCost)}</p>
                  </div>
                  <div>
